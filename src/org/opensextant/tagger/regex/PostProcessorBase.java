 ...
 * which perform de-duping or disambiguation type postprocessing. It provides
 * the base logic for this type of processing which consists of finding all sets
 * of "interacting" annotations (those that overlap, contain/are contained by
 * each other), sorting each interacting set using a Comparator (specified by
 * the extending subclass) and then selecting the first (top ranked) annotation
 * in each interacting set. This base class is extended by providing an
 * implementation of the getComparator() method. The comparator returned by this
 * method is used to sort each interacting set and thus determines which
 * annotation is selected from each interacting set.
 */
public abstract class PostProcessorBase implements PostProcessor {

	/**
	 * Find all sets of interacting (overlapping, containing/contained by)
	 * RegexAnnotations.
	 * 
	 * @param annos
	...
	public List<List<Match>> findInteractions(List<Match> annos) {
		...
		// sort the annotations into the order in which they appear in the
		// document
	...
	/**
	 * The decision logic. Sort each set of interacting annotations and pick the
	...
	/**
	 * Gets the comparator. Subclasses provide an implementation of this to
	 * supply the Comparator to be used in the decide() method.
	 * 
